...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[] ) {
	...
	if(nrhs < 3) mexErrMsgTxt("Valid input: [image], [x1, y1, x2, y2], bins, cutoffIntensity.");
    ...
    int cutoff = static_cast<int>(mxGetScalar(prhs[3]));

    Histogram hist_foreground(imgChannels.size(), bins);
    hist_foreground.extractForegroundHistogram(imgChannels, cv::Mat(), false, x1, y1, x2, y2, cutoff);
