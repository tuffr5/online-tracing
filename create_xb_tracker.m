function tracker = create_xb_tracker(img, init_bbox, init_params)

    if nargin < 3
        init_params = read_default_parameters();
    end
    ...
    % mask_bb history
    tracker.mask_budg_sz = init_params.mask_budg_sz;
    tracker.maskbb_size_budg = [];
    tracker.maskbb_hwratio_budg = [];
    tracker.maskbb_size_norm = 0;
    tracker.maskbb_size_sigma = 0;
    
    % cross section reliablity
    tracker.cross_plane_degree = init_params.cross_plane_degree;
    tracker.maskbb_size_sigma=init_params.maskbb_size_sigma;
    tracker.maskbb_hwratio_sigma=init_params.maskbb_hwratio_sigma;
    
    % for branch backtracking path cost
    tracker.branch_path_degree=init_params.branch_path_degree; 
    tracker.branch_color_sigma=init_params.branch_color_sigma;
    tracker.branch_bb_padding=init_params.branch_bb_padding;
    tracker.branch_counter=init_params.branch_counter;

    % segmentation parameters
    hist_lr = init_params.hist_lr;
    retain_rate = init_params.retain_rate;
    ...
    cutoff_intensity = init_params.cutoff_intensity;
    enable_alpha_mask=init_params.enable_alpha_mask;
    ...
    min_scale_factor = scale_step ^ ceil(log(max(8 ./ template_size_)) / log(scale_step));
    max_scale_factor = scale_step ^ floor(log(min([size(img,1) size(img,2)] ./ template_size_)) / log(scale_step));
    ...
    if use_segmentation
        ...
        % extract histograms
        hist_fg = mex_extractforeground(seg_img, obj_reg, nbins, cutoff_intensity);
        ...
        if enable_alpha_mask
            ...
            fg_trimap=fg_trimap.*(fg_trimap > 0 & mean(seg_patch, 3) > cutoff_intensity);
            % matting
    ...
    inds=find(mask);
    if ~isempty(inds)
        [y, x]=ind2sub(size(Y),inds(:));
        mask_bb=get_bb_smallest([x, y]);
    end
    
    tracker.last_maskbb=mask_bb;
    ...
    xs = get_scale_subwindow(img, c([2,1]), base_target_sz([2,1]), ...
        currentScaleFactor * scaleSizeFactors, scale_window, scale_model_sz([2,1]), [], w2c);
    ...
    tracker.enable_alpha_mask = enable_alpha_mask;
    tracker.bb = bb;
    ...
    tracker.c = c;
    tracker.nbins = nbins;
    tracker.cutoff_intensity = cutoff_intensity;
    ...
    if use_segmentation
        ...
        % to solve color drift
        tracker.src_hist_fg = hist_fg;
        tracker.src_hist_bg = hist_bg;
        
        tracker.hist_lr = hist_lr;
        tracker.retain_rate = retain_rate;
