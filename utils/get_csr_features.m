...
num_feat_ch = 0;
feat_gray = false; feat_hog = false; feat_cn = false; feat_hsv = false;
...
if feat_hsv
    % resize it to out size
	im_patch = imresize(im, out_size);
    im_patch_hsv = rgb2hsv(im_patch);
    % put color channels into output structure
    out(:, :, channel_id) = im_patch_hsv(:,:,1);
    channel_id = channel_id + 1;
end
